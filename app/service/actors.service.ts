...
const personMovieCreditsUrl = (personId) => `${baseUrl}/person/${personId}/movie_credits?api_key=${apiKey}`;
const personTvCreditsUrl = (personId) => `${baseUrl}/person/${personId}/tv_credits?api_key=${apiKey}`;
...
export class ActorService
{
    ...
    getPersonDetails<T extends Person>(personId: number): Observable<T>
    {
        ...
        return this.http.get(personDetailsUrl(personId)).flatMap(response => {
            ...
            return this.http.get(personExternalIdsUrl(personId)).flatMap(response =>
            {
                ...
                return this.http.get(personImagesUrl(personId)).flatMap(response =>
                {
                    this.mapImagesToImageArray(response.json().profiles, actor.profiles);

                    return this.getMovieCredits(actor).flatMap(response => {
                        return this.getTvCredits(actor).map(response => {
                            return actor;
                        });
                    });
                });
    ...
    public getMovieCredits<T extends Person>(person: T): Observable<T> {
        return this.http.get(personMovieCreditsUrl(person.id)).map(response =>
        {
    ...
    public getTvCredits<T extends Person>(person: T): Observable<T> {
        return this.http.get(personTvCreditsUrl(person.id)).map(response =>
        {
